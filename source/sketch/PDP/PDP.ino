...
/* misc constants*/
#define debugLEVEL 101 //0 most verbose, 20 operational description, 100 serial logging
#define TRUE 1
#define FALSE 0
...
void operateGate(byte *id) {
  ...
  while(startOpen+openDelay>millis()) {
    if(tim+TIMFLASH<millis()) {
      ...
      if(startOpen+TIMGATE<millis()) 
        digitalWrite(GATE, HIGH);
      if ((startOpen+(TIMGATE*2)<millis())) {
        byte card[ID_BYTES] = {0,0,0,0};
        byte *x = card;
        x = readRFID(x);
        if(x[0]>0 & inMemory(x)) { 
          debug(String("Early close during opening"),5);
          earlyClose = TRUE;
  ...
  while(startWait+TIMWAIT>millis()) {
    if(startWait+TIMGATE<millis()) 
      digitalWrite(GATE, HIGH);
    if ((startWait+(TIMGATE*2)<millis())) {
      byte card[ID_BYTES] = {0,0,0,0};
      byte *y = card;
      y = readRFID(y);
      if((y[0]>0 & inMemory(y)) | earlyClose) {
        debug(String("Early close during waiting"),5);
        break;}
      }
    }
  /*
